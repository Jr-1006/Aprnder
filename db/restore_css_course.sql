-- Restore CSS Flexbox & Grid Course for Cheska Diaz
USE pbl_gamified;

-- Get Cheska's mentor ID
SET @mentor_id = (SELECT id FROM users WHERE email = 'cheska.diaz@aprnder.com' LIMIT 1);
...
VALUES (
  @mentor_id, 
  '🎉 Your CSS Flexbox & Grid Mastery course has been restored successfully!', 
  'success', 
  0, 
  NOW()
);
...
-- Success message
SELECT 
  'CSS Course Restored Successfully!' AS status,
  @course_id AS course_id,
  'Course: CSS Flexbox & Grid Mastery' AS course_name,
  '7 quests added' AS quests_added;
